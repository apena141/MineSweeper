...
namespace MineSweeper
{
    public partial class Form2 : Form
    {
        ...
        public Form2(String text,int row, int col,int size,int mines) : this() {
            ...
            foreach (int i in Enumerable.Range(0,row))
                foreach (int j in Enumerable.Range(0,col))
                {
                    ...
                    buttons[i][j].Location = new Point(size * i, size * j + 25);
            ...
            switch (e.Button)
            {
                ...
                case MouseButtons.Right:
                    ...
                    if(field.Flagged.Contains(click_x * buttons[0].Length + click_y))
                    {
                        ...
                        flagDisplay.Text = "" + (Int32.Parse(flagDisplay.Text) - 1);
                    }
                    else
                    {
        ...
        {
            // DECREASE THE # OF OPEN GAMES
            if (lost == true)
            {
                MessageBox.Show("You lost!");
                ((MainForm)this.Owner).openGamesText.Text = (Int32.Parse(((MainForm)this.Owner).openGamesText.Text) - 1) + "";
                return;
            }
            ((MainForm)this.Owner).openGamesText.Text = (Int32.Parse(((MainForm)this.Owner).openGamesText.Text) - 1) + "";
            MessageBox.Show("Your game was successfully closed: " + e.CloseReason.ToString());
        }

        private void Form2_FormClosing(object sender, FormClosingEventArgs e)
        {
            // TO SEND A CONFIRMATION THAT THEY WANT TO CLOSE THE GAME
            if(this.field.Started && !this.field.Win() && !lost)
            {
                DialogResult result = MessageBox.Show("Are you sure you want to close the game?", "Game is in progress", MessageBoxButtons.YesNoCancel);
                e.Cancel = (result == DialogResult.No || result == DialogResult.Cancel);
            }
        }
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
                timerDisplay.Text = "" + (Int32.Parse(timerDisplay.Text) + 1);
        }
